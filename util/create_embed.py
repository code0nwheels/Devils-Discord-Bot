...
async def create_game(game_id, cmd):
	is_game, game = await hockey.get_game(game_id)
...
async def help(admin, commands, prefix):
	embed = discord.Embed(title="Help", description="Lists the commands this bot has.", color=0x0080ff)
	file = discord.File(f"{IMAGES_ICONS}DevilsWhat.png", filename=f"DevilsWhat.png")
	embed.set_thumbnail(url=f"attachment://DevilsWhat.png")
	for command in commands:
		if (admin and 'ADMIN' in command.description) or 'ADMIN' not in command.description:
			embed.add_field(name=command, value=command.description, inline=False)
	embed.set_footer(text=f"{prefix}help")

	return file, embed

async def create(title, description, names, values, cmd):
