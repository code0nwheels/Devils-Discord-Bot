...
class Devils(commands.Cog):
	...
	async def _send_report(self, ctx, message_obj, violation):
		...
		await channel.send(f"{admin_role.mention} {moderator_role.mention}", embed=embed, view=view)
	...
	async def openhelp(self, ctx, brief_description: str = None):
		...
		await thread.send(f"{ctx.author.mention} {admin_role.mention} {moderator_role.mention}", embed=embed)
